...
final class QuizService
{
    ...
    {
        $this->advanceState();
        $activeGroupId=(int)($this->pdo->query("SELECT id FROM quiz_groups WHERE status='active' ORDER BY id DESC LIMIT 1")->fetchColumn()?:0);
        $questionStatement=$activeGroupId>0
            ?$this->pdo->prepare("SELECT q.*,t.artist,t.title,t.genre,t.bpm,t.camelot FROM quiz_questions q LEFT JOIN tracks t ON t.id=q.track_id WHERE q.group_id=? ORDER BY CASE WHEN q.status='betting' THEN 0 WHEN q.status='open' THEN 1 WHEN q.status='revealed' THEN 2 WHEN q.opened_at IS NOT NULL THEN 3 ELSE 4 END,COALESCE(q.opened_at,q.created_at) DESC,q.id DESC LIMIT 1")
            :$this->pdo->prepare("SELECT q.*,t.artist,t.title,t.genre,t.bpm,t.camelot FROM quiz_questions q LEFT JOIN tracks t ON t.id=q.track_id WHERE q.group_id IS NULL ORDER BY CASE WHEN q.status='betting' THEN 0 WHEN q.status='open' THEN 1 WHEN q.status='revealed' THEN 2 WHEN q.opened_at IS NOT NULL THEN 3 ELSE 4 END,COALESCE(q.opened_at,q.created_at) DESC,q.id DESC LIMIT 1");
        $questionStatement->execute($activeGroupId>0?[$activeGroupId]:[]);$question=$questionStatement->fetch()?:null;
        $groupId=$activeGroupId>0?$activeGroupId:($question&&!empty($question['group_id'])?(int)$question['group_id']:null);
        ...
        if($participant){$participant['points']=$this->participantScore((int)$participant['id'],$groupId);if($question){$bet=$this->participantBet((int)$question['id'],(int)$participant['id']);if($payload)$payload['bet']=$bet;}}
        if($participant && (string)($participant['status']??'active')!=='active')$payload=null;
        $fastestPlayer=$question&&(string)$question['status']==='closed'?$this->fastestCorrect((int)$question['id']):null;
        return ['question'=>$payload,'group'=>$this->publicGroup($groupId),'participant'=>$participant?:null,'participants'=>$control?$this->participants($question?(int)$question['id']:0,$groupId):[],'leaderboard'=>$this->leaderboard($groupId),'fastest_player'=>$fastestPlayer,'group_complete'=>$groupId!==null&&$question&&$this->groupComplete($groupId,(int)$question['id']),'server_time_ms'=>(int)round(microtime(true)*1000)];
    }
    ...
    public function groups(): array
    {
        ...
        return ['items'=>array_map(static fn(array $item): array=>['id'=>(int)$item['id'],'name'=>(string)$item['name'],'event_date'=>(string)($item['event_date']??''),'description'=>(string)$item['description'],'image_path'=>(string)($item['image_path']??''),'status'=>(string)$item['status'],'question_count'=>(int)$item['question_count']],$items),'ungrouped_count'=>$ungrouped];
    }
    ...
    public function duplicateGroup(int $id,string $name,string $eventDate=''): array
    {
        ...
        $this->pdo->beginTransaction();
        try{
            $insert=$this->pdo->prepare("INSERT INTO quiz_groups(name,event_date,description,image_path,status) VALUES(?,?,?,?,'planned')");$insert->execute([$name,$eventDate?:null,(string)$group['description'],'']);$newId=(int)$this->pdo->lastInsertId();
    ...
    private function publicGroup(?int $groupId): ?array{$statement=$groupId?$this->pdo->prepare('SELECT id,name,event_date,image_path FROM quiz_groups WHERE id=?'):$this->pdo->prepare("SELECT id,name,event_date,image_path FROM quiz_groups WHERE status='active' ORDER BY id DESC LIMIT 1");$statement->execute($groupId?[$groupId]:[]);$row=$statement->fetch();if(!$row)return null;$imagePath=(string)($row['image_path']??'');$baseUrl=rtrim((string)setting('hosting_base_url',''),'/');if($baseUrl===''){$scheme=!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off'?'https':'http';$baseUrl=$scheme.'://'.($_SERVER['HTTP_HOST']??'localhost').rtrim(str_replace('\\','/',dirname($_SERVER['SCRIPT_NAME']??'/')),'/');}return ['id'=>(int)$row['id'],'name'=>(string)$row['name'],'event_date'=>(string)($row['event_date']??''),'image_url'=>$imagePath!==''?$baseUrl.'/'.$imagePath:''];}
    ...
    private function fastestCorrect(int $questionId): ?array{$statement=$this->pdo->prepare('SELECT p.display_name,a.response_ms FROM quiz_answers a INNER JOIN quiz_participants p ON p.id=a.participant_id WHERE a.question_id=? AND a.is_correct=1 ORDER BY a.response_ms,a.id LIMIT 1');$statement->execute([$questionId]);$row=$statement->fetch();return $row?['display_name'=>(string)$row['display_name'],'bonus_points'=>self::FASTEST_CORRECT_BONUS,'response_ms'=>(int)$row['response_ms']]:null;}
    ...
    private function groupComplete(int $groupId,int $questionId): bool{$statement=$this->pdo->prepare("SELECT q.status,q.sort_order,(SELECT MAX(last.sort_order) FROM quiz_questions last WHERE last.group_id=q.group_id) last_order FROM quiz_questions q WHERE q.group_id=? AND q.id=?");$statement->execute([$groupId,$questionId]);$row=$statement->fetch();return $row&&(string)$row['status']==='closed'&&(int)$row['sort_order']>=(int)$row['last_order'];}
    ...
    private function participants(int $questionId,?int $groupId): array{$statement=$this->pdo->prepare("SELECT p.id,p.display_name,p.status,p.rejoin_requested_at,IF(p.status='active' AND p.is_online=1 AND p.last_seen_at>=DATE_SUB(NOW(),INTERVAL 8 SECOND),1,0) online,p.last_seen_at,p.left_at,a.selected_option,COALESCE(a.is_correct,0) is_correct,COALESCE(a.points,0) points,b.mode bet_mode,COALESCE(b.stake_points,0) bet_points,b.status bet_status FROM quiz_participants p LEFT JOIN quiz_answers a ON a.participant_id=p.id AND a.question_id=? LEFT JOIN quiz_bets b ON b.participant_id=p.id AND b.question_id=? WHERE p.status<>'removed'");$statement->execute([$questionId,$questionId]);$items=$statement->fetchAll();foreach($items as &$item)$item['total_points']=$this->participantScore((int)$item['id'],$groupId);unset($item);usort($items,static fn(array $left,array $right): int=>((int)$right['total_points']<=>(int)$left['total_points'])?:strnatcasecmp((string)$left['display_name'],(string)$right['display_name']));return $items;}
