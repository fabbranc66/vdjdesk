...
final class QuizService
{
    ...
    public function state(string $token='',bool $control=false): array
    {
        ...
        $groupId=$question&&!empty($question['group_id'])?(int)$question['group_id']:null;
        return ['question'=>$payload,'participant'=>$participant?:null,'participants'=>$control?$this->participants($question?(int)$question['id']:0,$groupId):[],'leaderboard'=>$this->leaderboard($groupId),'group_complete'=>$groupId!==null&&$question&&$this->groupComplete($groupId,(int)$question['id']),'server_time_ms'=>(int)round(microtime(true)*1000)];
    }
