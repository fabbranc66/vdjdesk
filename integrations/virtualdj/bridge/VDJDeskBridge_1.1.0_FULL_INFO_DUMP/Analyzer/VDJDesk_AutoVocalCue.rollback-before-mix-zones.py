...
def analyze_urban_or_cache(
    ...
    key = (
        f"genre-profile-v8.0|{profile}|{normalize_genre(genre)}|{str(stem).lower()}|"
        f"{bpm:.6f}|{marker}"
    )
    ...
    build_drop_pairs = (
        analyze_build_drop_sections(
            stem,
            bpm,
            no_kick_analysis.get("sections", []),
        )
        if profile == "edm_house"
        else []
    )
    stream_matrix = build_vocal_kick_matrix(
...
def show_analysis_summary(
    ...
    matrix = analysis.get("streamMatrix", [])
    lines.extend(["", f"SOVRAPPOSIZIONI VOCAL / KICK ({len(matrix)})"])
    lines.extend(
        f"  {index}. {_summary_time(section['time'])} -> "
        f"{_summary_time(section['end'])}  {section['label']}"
        for index, section in enumerate(matrix, start=1)
    )
    if not matrix:
        lines.append("  Nessuna sezione")
...
def process_track(
    ...
    if profile == "edm_house":
        log("Modalita': NOVOX + NOKICK + BUILD/DROP + MATRICE / WRITE CUES")
    else:
        log("Modalita': NOVOX + NOKICK + MATRICE / WRITE CUES")
