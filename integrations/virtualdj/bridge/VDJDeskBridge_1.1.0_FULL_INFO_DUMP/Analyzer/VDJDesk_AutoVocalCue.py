...
STREAM_MATRIX_MIN_BEATS = 4
INSTRUMENT_THRESHOLD_RATIO = 0.35
BASS_THRESHOLD_RATIO = 0.35
INSTRUMENT_STATE_MIN_BEATS = 4
MIX_ZONE_MIN_BEATS = 8
MIX_ZONE_LONG_BEATS = 16
URBAN_POP_GENRES = (
    "dance pop",
    "latin urban",
    "brazilian funk",
...
def analyze_no_kick_sections(stem_path: Path, bpm: float) -> dict:
    ...
    strong_kick_frames = np.flatnonzero(kick_mask)
    first_kick_time = (
        float(strong_kick_frames[0] * seconds_per_frame)
        if strong_kick_frames.size
        else None
    )
    ...
    log(
        "FIRST KICK: "
        + (f"{first_kick_time:.3f}s" if first_kick_time is not None else "assente")
    )
    for idx, section in enumerate(sections, start=1):
        log(
            f"NOKICK {idx}: "
    ...
    return {
        "sections": sections,
        "duration": len(kick) / SR,
        "firstKickTime": first_kick_time,
    }
...
def build_vocal_kick_matrix(
    duration: float,
    bpm: float,
    novox_sections: List[dict],
    nokick_sections: List[dict],
) -> List[dict]:
    beat_seconds = 60.0 / max(bpm, 1.0)
    beat_count = max(1, int(np.ceil(duration / beat_seconds)))

    def _inside(time_value: float, sections: List[dict]) -> bool:
        return any(
            float(section["time"]) <= time_value < float(section["end"])
            for section in sections
        )

    beat_states = []
    for beat_index in range(beat_count):
        start = beat_index * beat_seconds
        end = min(duration, start + beat_seconds)
        midpoint = (start + end) / 2.0
        vocal = not _inside(midpoint, novox_sections)
        kick = not _inside(midpoint, nokick_sections)
        if vocal and kick:
            label = "VOCAL + KICK"
        elif vocal:
            label = "VOCAL + NOKICK"
            ...
            label = "NOVOX + NOKICK"
        beat_states.append((start, end, label, vocal, kick))

    matrix = []
    for start, end, label, vocal, kick in beat_states:
        if matrix and matrix[-1]["label"] == label:
            matrix[-1]["end"] = end
            matrix[-1]["beats"] += 1
            continue
        matrix.append(
    ...
    for section in matrix:
        if section["beats"] < STREAM_MATRIX_MIN_BEATS and filtered:
            filtered[-1]["end"] = section["end"]
            filtered[-1]["beats"] += section["beats"]
            continue
        filtered.append(section)
    if len(filtered) > 1 and filtered[0]["beats"] < STREAM_MATRIX_MIN_BEATS:
        filtered[1]["time"] = filtered[0]["time"]
        filtered[1]["beats"] += filtered[0]["beats"]
        filtered.pop(0)
    matrix = filtered

    log(f"MATRICE VOCAL/KICK: sections={len(matrix)}")
    for index, section in enumerate(matrix, start=1):
        log(
            f"MATRIX {index}: {section['label']} | "
            f"{section['time']:.3f}s -> {section['end']:.3f}s | "
            f"beat={section['beats']}"
        )
    return matrix


def _stabilize_beat_states(states: np.ndarray, minimum_beats: int) -> np.ndarray:
    stable = states.astype(bool).copy()
    for _ in range(3):
        state_runs = list(runs(stable))
        changed = False
        for index, (value, start, end) in enumerate(state_runs):
            if end - start >= minimum_beats:
                continue
            previous_run = state_runs[index - 1] if index > 0 else None
            next_run = state_runs[index + 1] if index + 1 < len(state_runs) else None
            if previous_run and next_run and previous_run[0] == next_run[0]:
                stable[start:end] = previous_run[0]
            elif previous_run and next_run:
                previous_length = previous_run[2] - previous_run[1]
                next_length = next_run[2] - next_run[1]
                stable[start:end] = (
                    previous_run[0]
...
def analyze_stream_presence(
    stem_path: Path,
    bpm: float,
    stream_title: str,
    threshold_ratio: float,
) -> dict:
    stream = probe_optional_stream(stem_path, stream_title)
    if stream is None:
        raise RuntimeError(
            f"Stream {stream_title} non trovato nello stem VirtualDJ."
        )

    audio = decode_stream(stem_path, stream)
    frame = 2048
    hop = 512
    seconds_per_frame = hop / SR
    _, rms_db = _frame_rms_db(audio, frame, hop)
    finite = rms_db[np.isfinite(rms_db)]
    noise_db = float(np.percentile(finite, 10.0))
    signal_db = float(np.percentile(finite, 90.0))
    threshold_db = noise_db + threshold_ratio * max(
        signal_db - noise_db,
        6.0,
    )
    ...
    beat_seconds = 60.0 / max(bpm, 1.0)
    beat_count = max(1, int(np.ceil(duration / beat_seconds)))
    states = np.zeros(beat_count, dtype=bool)

    for beat_index in range(beat_count):
        start_frame = int(beat_index * beat_seconds / seconds_per_frame)
        end_frame = min(
            len(rms_db),
            int(np.ceil((beat_index + 1) * beat_seconds / seconds_per_frame)),
        )
        values = rms_db[start_frame:end_frame]
        if values.size:
            states[beat_index] = float(np.percentile(values, 75.0)) >= threshold_db

    states = _stabilize_beat_states(states, INSTRUMENT_STATE_MIN_BEATS)
    sections = []
    for present, start, end in runs(states):
        sections.append(
            {
                "time": start * beat_seconds,
                "end": min(duration, end * beat_seconds),
                "beats": end - start,
    ...
    log(
        f"{stream_title.upper()} STEM {stream}: "
        f"noiseDb={noise_db:.2f} | signalDb={signal_db:.2f} | "
        f"thresholdDb={threshold_db:.2f} | sections={len(sections)}"
    )
    return {
        "sections": sections,
        "duration": duration,
...
def build_mix_zones(
    ...
    novox_sections: List[dict],
    nokick_sections: List[dict],
    instrument_sections: List[dict],
    bass_sections: List[dict],
) -> List[dict]:
    beat_seconds = 60.0 / max(bpm, 1.0)
    beat_count = max(1, int(np.ceil(duration / beat_seconds)))
    ...
    def _stream_present(time_value: float, sections: List[dict]) -> bool:
        return any(
            section.get("present", False)
            and float(section["time"]) <= time_value < float(section["end"])
            for section in sections
        )

    labels = []
    for beat_index in range(beat_count):
        midpoint = min(duration, (beat_index + 0.5) * beat_seconds)
        vocal = not _inside(midpoint, novox_sections)
        kick = not _inside(midpoint, nokick_sections)
        instruments = _stream_present(midpoint, instrument_sections)
        bass = _stream_present(midpoint, bass_sections)
        label = None
        code = None
        if kick and not vocal and not instruments and not bass:
            label, code = "SOLO KICK", "KICK"
        elif vocal and not kick and not instruments and not bass:
            label, code = "SOLO VOCAL", "VOCAL"
        elif vocal and kick and not instruments and not bass:
            label, code = "VOCAL + KICK", "VOX+KICK"
        elif vocal and instruments and bass and not kick:
            label, code = "VOCAL + INSTR", "VOX+INSTR"
        labels.append((label, code))

    zones = []
    start = 0
    while start < beat_count:
        label, code = labels[start]
        end = start + 1
        while end < beat_count and labels[end] == labels[start]:
            end += 1
        beats = end - start
        if label is not None and beats >= MIX_ZONE_MIN_BEATS:
            extension = (
                MIX_ZONE_LONG_BEATS
                if beats >= MIX_ZONE_LONG_BEATS
                else MIX_ZONE_MIN_BEATS
            )
            zones.append(
                {
                    "time": start * beat_seconds,
    ...
    log(f"ZONE MIX: sections={len(zones)}")
    for index, zone in enumerate(zones, start=1):
        log(
            f"MIX {index}: {zone['label']} | {zone['time']:.3f}s -> "
            f"{zone['end']:.3f}s | beat={zone['beats']} | "
            f"extension={zone['extension']}"
        )
    return zones
...
def build_phrase_windows(
    duration: float,
    bpm: float,
    first_kick_time: Optional[float],
    novox_sections: List[dict],
    nokick_sections: List[dict],
    instrument_sections: List[dict],
    bass_sections: List[dict],
) -> List[dict]:
    if first_kick_time is None:
        return []

    beat_seconds = 60.0 / max(bpm, 1.0)
    window_16 = 16.0 * beat_seconds
    window_8 = 8.0 * beat_seconds
    anchor = min(max(0.0, float(first_kick_time)), duration)
    windows = []

    full_intro_windows = int(anchor // window_16)
    intro_remainder = anchor - full_intro_windows * window_16
    if intro_remainder >= window_8:
        windows.append(
            {
                "time": intro_remainder - window_8,
                "end": intro_remainder,
                "extension": 8,
                "phase": "INTRO",
            }
        )
    for index in range(full_intro_windows):
        start = intro_remainder + index * window_16
        windows.append(
            {
                "time": start,
                "end": start + window_16,
    ...
    cursor = anchor
    while duration - cursor >= window_16:
        windows.append(
            {
                "time": cursor,
                "end": cursor + window_16,
                "extension": 16,
                "phase": "BRANO",
            }
        )
        cursor += window_16
    if duration - cursor >= window_8:
        windows.append(
            {
                "time": cursor,
                "end": cursor + window_8,
    ...
    for window in windows:
        states = []
        beat_count = int(window["extension"])
        for beat_index in range(beat_count):
            midpoint = (
                float(window["time"])
                + (beat_index + 0.5) * beat_seconds
            )
            states.append(
                (
                    not _inside(midpoint, novox_sections),
                    not _inside(midpoint, nokick_sections),
                    _stream_present(midpoint, instrument_sections),
                    _stream_present(midpoint, bass_sections),
                )
            )
        ...
        dominant_state, dominant_count = max(
            counts.items(),
            key=lambda item: item[1],
        )
        names = ["VOCAL", "KICK", "INSTR", "BASS"]
        active_names = [
            name for name, present in zip(names, dominant_state)
            if present
        ]
        window["streams"] = active_names
        window["label"] = " + ".join(active_names) if active_names else "NESSUNO"
        window["confidence"] = dominant_count / max(1, len(states))

    log(f"FINESTRE FRASE: anchor={anchor:.3f}s | sections={len(windows)}")
    for index, window in enumerate(windows, start=1):
        log(
            f"WINDOW {index}: {window['phase']} {window['extension']}B | "
            f"{window['time']:.3f}s -> {window['end']:.3f}s | "
            f"{window['label']} | confidence={window['confidence']:.2f}"
        )
    return windows
...
def analyze_urban_or_cache(
    ...
    key = (
        f"phrase-pair-cues-v12.0|{profile}|{normalize_genre(genre)}|{str(stem).lower()}|"
        f"{bpm:.6f}|{marker}"
    )
    ...
    build_drop_pairs = []
    stream_matrix = []
    instruments_analysis = analyze_stream_presence(
        stem,
        bpm,
        "instruments",
        INSTRUMENT_THRESHOLD_RATIO,
    )
    bass_analysis = analyze_stream_presence(
        stem,
        bpm,
        "bass",
        BASS_THRESHOLD_RATIO,
    )
    phrase_windows = build_phrase_windows(
        float(no_kick_analysis.get("duration", 0.0)),
        bpm,
        no_kick_analysis.get("firstKickTime"),
        analysis.get("sections", []),
        no_kick_analysis.get("sections", []),
        instruments_analysis.get("sections", []),
        bass_analysis.get("sections", []),
    )
    mix_zones = []
    ...
    for zone in mix_zones:
        code = str(zone["code"])
        mix_counts[code] = mix_counts.get(code, 0) + 1
        cue_events.append(
            {
                "time": float(zone["time"]),
                "type": "mix_zone",
                "name": (
                    f"MIX{zone['extension']} {code} {mix_counts[code]}"
                ),
                "color": mix_colors.get(code, "white"),
                "score": float(zone["beats"]),
                "end": float(zone["end"]),
                "priority": 10,
            }
        )
    ...
    for window in phrase_windows:
        streams = list(window.get("streams", []))
        if len(streams) != 2:
            continue
        pair_code = "+".join(stream_codes[stream] for stream in streams)
        pair_counts[pair_code] = pair_counts.get(pair_code, 0) + 1
        cue_events.append(
            {
                "time": float(window["time"]),
                "type": "stream_pair",
                "name": (
                    f"MIX{window['extension']} {pair_code} "
                    f"{pair_counts[pair_code]}"
                ),
                "color": pair_colors.get(pair_code, "white"),
                "score": float(window["confidence"]),
                "priority": 10,
            }
        )
    cue_events.sort(key=lambda x: x["time"])
    ...
    result = {
        ...
        "instrumentSections": instruments_analysis.get("sections", []),
        "bassSections": bass_analysis.get("sections", []),
        "phraseWindows": phrase_windows,
        "mixZones": mix_zones,
...
def write_cues(deck: int, boundaries: List[dict]) -> List[dict]:
    selected = sorted(
        sorted(
            boundaries,
            key=lambda point: float(point.get("priority", 0.0)),
            reverse=True,
        )[:MAX_CUES],
        key=lambda point: float(point.get("time", 0.0)),
    )
...
def show_analysis_summary(
    ...
    profile = str(analysis.get("analysisProfile") or "-")
    profile_label = {
        "urban_pop": "Urban / Pop",
        "edm_house": "EDM / House",
    }.get(profile, profile)
    lines = [
        track_label,
        f"Genere: {genre or '-'}",
        f"Profilo: {profile_label}",
        "",
    ]

    windows = analysis.get("phraseWindows", [])
    lines.append(f"FINESTRE MUSICALI ({len(windows)})")
    lines.extend(
        f"  {index}. {_summary_time(window['time'])} -> "
        f"{_summary_time(window['end'])}  {window['phase']} "
        f"{window['extension']}B  {window['label']}  "
        f"[{window['confidence']:.0%}]"
        for index, window in enumerate(windows, start=1)
    )
    if not windows:
        lines.append("  Nessuna finestra")

    lines.extend(["", f"CUE COPPIE SCRITTI: {len(written)}"])
    message = "\n".join(lines)
...
def analysis_profile_for_genre(value: str) -> Optional[str]:
    genre = normalize_genre(value)
    if any(pattern in genre for pattern in URBAN_POP_GENRES):
        return "urban_pop"
    if any(pattern in genre for pattern in EDM_HOUSE_GENRES):
        return "edm_house"
    return None
...
def process_track(
    ...
    if INFO_ONLY_MODE:
        log("Modalita': SOLO INFO BRANO / NESSUNA ANALISI / NESSUN CUE")
        log("--- END INFO ---")
        log(f"DECK {deck_num}: informazioni acquisite.")
        return

    profile = analysis_profile_for_genre(genre)
    if profile is None:
        log("--- END INFO ---")
        log("Genere fuori whitelist: analisi non eseguita.")
        return

    profile_label = "URBAN/POP" if profile == "urban_pop" else "EDM/HOUSE"
    log(f"Profilo analisi: {profile_label}")
    log("Modalita': FINESTRE 8/16 BEAT / CUE SOLO COPPIE STREAM")
    ...
    try:
        analysis = analyze_urban_or_cache(stem, bpm, genre, profile)
    except Exception as exc:
        log("--- END INFO ---")
    ...
    clear_cues(deck_num)
    try:
        written = write_cues(deck_num, boundaries) if boundaries else []
        save_result(deck_num, audio_path, stem, analysis, written)
    except Exception as exc:
        log("--- END INFO ---")
        log(f"Scrittura cue coppie fallita: {exc}")
        return

    log("--- END INFO ---")
    log(f"Cue coppie scritti: {len(written)}")
    for cue in written:
        log(
