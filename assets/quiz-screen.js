...
async function refreshScreenAvailability(){try{const chillFrame=document.querySelector('#chill-reel-frame');const modeResponse=await fetch('api.php?action=game-mode-state',{cache:'no-store'});if(modeResponse.ok){const mode=await modeResponse.json();const showChill=mode.active_game==='chill_reel';chillFrame.hidden=!showChill;if(showChill)return}const response=await fetch('api.php?action=public-modules',{cache:'no-store'});if(!response.ok)return;const data=await response.json();screenQuizEnabled=!!data.quiz_enabled;if(screenQuizEnabled)chillFrame.hidden=true;if(!screenQuizEnabled){renderQuizDisabled();return}refreshScreen()}catch(error){}}
