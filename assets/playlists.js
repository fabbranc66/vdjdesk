...
function applyPlaylistFilters(){
  const query=$('#playlist-search').value.trim().toLocaleLowerCase('it'),bpm=Number($('#playlist-bpm').value||0),key=$('#playlist-key').value.trim().toLocaleLowerCase('it'),genre=$('#playlist-genre').value.trim().toLocaleLowerCase('it'),year=Number($('#playlist-year').value||0),quality=$('#playlist-quality').value;
  state.tracks=playlistAllTracks.filter(track=>{
    ...
    return(!query||text.includes(query))&&(!bpm||Math.abs(Number(track.bpm||0)-bpm)<=8)&&(!key||String(track.camelot||track.musical_key||'').toLocaleLowerCase('it').includes(key))&&(!genre||String(track.genre||'').toLocaleLowerCase('it').includes(genre))&&(!year||Number(track.year)===year)&&(!quality||(quality==='standard'?standard:quality==='below'?!standard&&!video:video));
  });
  renderTracks();
}
