...
function spotifyFailureCountDetail(count){return `Fallimenti ${count}`}
...
const hasValidSpotifyId=track=>/^[A-Za-z0-9]{22}$/.test(String(track.spotify_id||''));
const pendingSpotifyTracks=()=>state.tracks.filter(track=>Number(track.id)>0&&hasValidSpotifyId(track)&&(['never','error'].includes(track.spotify_features_status)||(!track.spotify_features_status&&!track.spotify_features_updated_at)));
const unidentifiedSpotifyTracks=()=>state.tracks.filter(track=>Number(track.id)>0&&!hasValidSpotifyId(track));
...
document.addEventListener('click',async event=>{const button=event.target.closest('.save-spotify-link');if(!button)return;const input=button.closest('.spotify-link-editor').querySelector('input');button.disabled=true;try{const result=await post('spotify-link-update',{id:Number(button.dataset.id),url:input.value.trim()});input.value=result.track.spotify_url;const index=state.tracks.findIndex(track=>Number(track.id)===Number(button.dataset.id));if(index>=0)state.tracks[index]=result.track;window.refreshVdjPlaylistTrack?.(result.track);toast('Link Spotify salvato nel database')}catch(error){toast(error.message)}finally{button.disabled=false}});

let spotifyClipboardTimer=null;
function closeSpotifySearchWindow(spotifyWindow){if(!spotifyWindow)return;try{spotifyWindow.location.href='about:blank';setTimeout(()=>{try{spotifyWindow.close()}catch(error){}},120)}catch(error){try{spotifyWindow.close()}catch(closeError){}}}
document.addEventListener('click',async event=>{const link=event.target.closest('.spotify-search-link');if(!link)return;event.preventDefault();const url=link.href;const trackId=Number(link.dataset.trackId);const force=link.dataset.force==='1';delete link.dataset.force;const spotifyWindow=window.open('about:blank','vdjdesk_spotify');try{await post('spotify-clipboard-start',{id:trackId,force});toast(`${force?'ID e link ignorati - ':''}appunti azzerati - copia il link del brano da Spotify`);if(spotifyWindow){spotifyWindow.location.href=url;spotifyWindow.focus()}clearInterval(spotifyClipboardTimer);spotifyClipboardTimer=setInterval(async()=>{try{const result=await api(`spotify-clipboard-status&id=${trackId}`);if(result.pending)return;clearInterval(spotifyClipboardTimer);spotifyClipboardTimer=null;closeSpotifySearchWindow(spotifyWindow);window.focus();if(result.saved){const index=state.tracks.findIndex(track=>Number(track.id)===Number(result.track.id));if(index>=0)state.tracks[index]=result.track;const refreshed=window.refreshVdjPlaylistTrack?.(result.track);if(result.duplicate){const currentPath=result.track.file_path||'';const existingPath=result.duplicate.file_path||'';const remove=window.confirm(`Spotify ID già presente nella libreria.\n\nCOPIA CORRENTE:\n${currentPath}\n\nGIÀ PRESENTE:\n${existingPath}\n\nEliminare fisicamente la copia corrente?`);if(remove){await post('duplicate-file-delete',{path:currentPath});if(!refreshed)await loadTracks(true,false);toast('Copia corrente eliminata dalla libreria');return}if(!refreshed)await loadTracks(true,false);toast('Cancellazione annullata · copia mantenuta come doppione');return}if(!refreshed)await loadTracks(true,false);toast('Link Spotify acquisito e salvato nel database')}else toast('Acquisizione link Spotify scaduta')}catch(error){clearInterval(spotifyClipboardTimer);spotifyClipboardTimer=null;closeSpotifySearchWindow(spotifyWindow);window.focus();toast(error.message)}},1200)}catch(error){closeSpotifySearchWindow(spotifyWindow);toast(error.message)}});
...
document.addEventListener('click',async event=>{const link=event.target.closest('.spotmate-link');if(!link||link.closest('#vdj-playlist-results'))return;event.preventDefault();try{const watch=await post('replacement-watch-start',{id:Number(link.dataset.trackId)});await navigator.clipboard.writeText(link.dataset.spotifyUrl);toast(`SpotMate attivo - scarica in Downloads; poi sposto in ${watch.staging_folder||'Da_classificare'}`);const spotmateWindow=window.open(link.href,'vdjdesk_spotmate');spotmateWindow?.focus();clearInterval(replacementWatchTimer);replacementWatchTimer=setInterval(async()=>{try{await handleReplacementWatchResult(await api('replacement-watch-status'),spotmateWindow)}catch(error){clearInterval(replacementWatchTimer);replacementWatchTimer=null;toast(error.message)}},1500)}catch(error){toast(error.message)}});

function updateBulkSpotifyButton(){
  if(bulkSpotifyButton.disabled)return;
  const withId=state.tracks.filter(hasValidSpotifyId).length;
  const pending=pendingSpotifyTracks().length;
  setGlobalActionIcon(bulkSpotifyButton,krIcon.metrics,'Recupera metriche Spotify sui brani visibili',`${pending}/${withId} da ricercare`);
...
function decorateSpotifyTracks(){
  $$('.track-row').forEach(row=>{
    ...
    const track=state.tracks.find(item=>item.id===trackId)||window.vdjPlaylistTrackLookup?.get?.(trackId);
    if(!track)return;
    const validSpotifyId=hasValidSpotifyId(track);
    const spotifyReady=validSpotifyId&&Number(track.file_exists)!==0&&['complete','partial'].includes(String(track.spotify_features_status||''));
    row.classList.toggle('has-spotify-id',spotifyReady);
    ...
    if(!row.querySelector('.spotify-title-actions')){
      ...
      const spotify=document.createElement('a');spotify.className='spotify-title-action spotify-open spotify-direct-link spotify-search-link';spotify.href=validSpotifyId?(track.spotify_url||`https://open.spotify.com/track/${track.spotify_id}`):`https://open.spotify.com/search/${encodeURIComponent(query)}`;spotify.target='vdjdesk_spotify';spotify.dataset.trackId=track.id;spotify.textContent='S';spotify.title=validSpotifyId?'Apri il brano su Spotify':'Cerca il brano su Spotify e acquisisci il link copiato';spotify.setAttribute('aria-label',spotify.title);actions.append(spotify);
      if(validSpotifyId){const spotmate=document.createElement('a');spotmate.className='spotify-title-action spotify-spotmate spotmate-link';spotmate.href='https://spotmate.online/premium';spotmate.target='vdjdesk_spotmate';spotmate.dataset.trackId=track.id;spotmate.dataset.spotifyUrl=track.spotify_url||`https://open.spotify.com/track/${track.spotify_id}`;spotmate.textContent='S';spotmate.title='Apri SpotMate, monitora il download e sostituisci il file';spotmate.setAttribute('aria-label',spotmate.title);actions.append(spotmate)}
      ...
      (row.closest('#playlist-results')?row.querySelector('.playlist-title-actions'):row.querySelector('.track-identity strong'))?.append(actions);
    }
    if(row.closest('#library-results')&&validSpotifyId){
      const actions=row.querySelector('.spotify-title-actions');
      if(actions&&!actions.querySelector('.spotify-spotmate')){
        const spotmate=document.createElement('a');spotmate.className='spotify-title-action spotify-spotmate spotmate-link';spotmate.href='https://spotmate.online/premium';spotmate.target='vdjdesk_spotmate';spotmate.dataset.trackId=track.id;spotmate.dataset.spotifyUrl=track.spotify_url||`https://open.spotify.com/track/${track.spotify_id}`;spotmate.textContent='S';spotmate.title='Apri SpotMate, monitora il download e sostituisci il file';spotmate.setAttribute('aria-label',spotmate.title);actions.append(spotmate);
      }
    }
    if(!row.closest('#vdj-playlist-results')){
      if(validSpotifyId&&!row.querySelector('.spotify-id-badge')){const badge=document.createElement('span');badge.className='badge blue spotify-id-badge';const labels={complete:'Spotify metriche',partial:'Spotify parziali',unavailable:'Spotify non disponibili',error:'Spotify da riprovare'};badge.textContent=labels[track.spotify_features_status]||'Spotify ID';row.querySelector('.track-tags')?.prepend(badge)}
      if(validSpotifyId&&!row.querySelector('.spotify-direct-link')){const link=document.createElement('a');link.className='badge blue spotify-direct-link';link.href=track.spotify_url||`https://open.spotify.com/track/${encodeURIComponent(track.spotify_id)}`;link.target='vdjdesk_spotify';link.textContent='Spotify';link.title='Apri il brano nella scheda Spotify Web controllata da KR Desk';row.querySelector('.track-tags')?.prepend(link)}
      if(validSpotifyId&&!row.querySelector('.spotmate-link')){const link=document.createElement('a');link.className='badge spotify-direct-link spotmate-link';link.href='https://spotmate.online/premium';link.target='_blank';link.rel='noopener';link.dataset.trackId=track.id;link.dataset.spotifyUrl=track.spotify_url||`https://open.spotify.com/track/${track.spotify_id}`;link.textContent='SpotMate';link.title='Copia il link, monitora il download e sostituisci automaticamente';row.querySelector('.track-tags')?.prepend(link)}
      if(!validSpotifyId&&!row.querySelector('.spotify-search-link')){const query=[track.artist,track.title].filter(Boolean).join(' ');const link=document.createElement('a');link.className='badge spotify-direct-link spotify-search-link';link.href=`https://open.spotify.com/search/${encodeURIComponent(query)}`;link.target='vdjdesk_spotify';link.dataset.trackId=track.id;link.textContent='Cerca Spotify';link.title='Apri la ricerca nella scheda Spotify Web controllata da KR Desk';row.querySelector('.track-tags')?.prepend(link)}
    }else if(validSpotifyId&&!row.querySelector('.spotify-id-badge')){
      const badge=document.createElement('span');badge.className='badge blue spotify-id-badge';const labels={complete:'Spotify metriche',partial:'Spotify parziali',unavailable:'Spotify non disponibili',error:'Spotify da riprovare'};badge.textContent=labels[track.spotify_features_status]||'Spotify ID';row.querySelector('.track-tags')?.prepend(badge);
    }
    ...
    $$('.auto-tag-badge',row).forEach(item=>item.remove());
    ...
    const moveButton=document.createElement('button');moveButton.dataset.action='library-move';moveButton.textContent='Sposta nella cartella genere';menu.append(moveButton);const chooseMoveButton=document.createElement('button');chooseMoveButton.dataset.action='library-move-choose';chooseMoveButton.textContent='Sposta in cartella...';menu.append(chooseMoveButton);
...
if($('#vdj-playlist-results'))spotifyFeaturesMenuObserver.observe($('#vdj-playlist-results'),{childList:true,subtree:true});

document.addEventListener('click',async event=>{const button=event.target.closest('[data-action="spotify-features"]');if(!button)return;const trackId=Number(button.closest('.track-row').dataset.id);const track=state.tracks.find(item=>item.id===trackId)||window.vdjPlaylistTrackLookup?.get?.(trackId);if(!track)return;try{toast('Recupero metriche Spotify...');const result=await post('spotify-audio-features',{id:track.id});Object.assign(track,result.track);const refreshed=window.refreshVdjPlaylistTrack?.(result.track);if(!refreshed)renderTracks();decorateSpotifyTracks();toast(`Spotify: energia ${Math.round(result.features.energy*100)} - ballabilita ${Math.round(result.features.danceability*100)} - ${Math.round(result.features.tempo)} BPM`)}catch(error){toast(error.message)}});
...
bulkSpotifyButton.addEventListener('click',async()=>{const tracks=pendingSpotifyTracks();if(!tracks.length){toast('Nessun brano con Spotify ID da aggiornare nella lista');return}bulkSpotifyButton.disabled=true;krProgress.start('Recupero metriche Spotify',tracks.length,'Lista filtrata');let completed=0,errors=0;for(let index=0;index<tracks.length;index++){const track=tracks[index];setGlobalActionIcon(bulkSpotifyButton,krIcon.metrics,'Recupero metriche Spotify',`${index+1}/${tracks.length}`);krProgress.update('Recupero metriche Spotify',index+1,tracks.length,spotifyFailureCountDetail(errors));try{let result;try{result=await post('spotify-audio-features',{id:track.id})}catch(error){if(!error.message.includes('Limite Spotify'))throw error;setGlobalActionIcon(bulkSpotifyButton,krIcon.metrics,'Pausa Spotify','30 secondi');krProgress.update('Pausa limite Spotify',index+1,tracks.length,'Attendo 30 secondi');await wait(30000);result=await post('spotify-audio-features',{id:track.id})}Object.assign(track,result.track);completed++}catch(error){errors++;track.spotify_features_status='error';krProgress.update('Recupero metriche Spotify',index+1,tracks.length,spotifyFailureCountDetail(errors))}if(index<tracks.length-1)await wait(950)}bulkSpotifyButton.disabled=false;renderTracks();decorateSpotifyTracks();updateBulkSpotifyButton();krProgress.done('Metriche Spotify completate',`${completed} ok${errors?` - ${errors} errori`:''}`,spotifyFailureCountDetail(errors));toast(`${completed} metriche Spotify salvate${errors?` - ${errors} errori`:''}`)});

identifySpotifyButton.addEventListener('click',async()=>{const tracks=unidentifiedSpotifyTracks();if(!tracks.length){toast('Tutti i brani della lista hanno gia uno Spotify ID');return}identifySpotifyButton.disabled=true;bulkSpotifyButton.disabled=true;krProgress.start('Ricerca Spotify ID',tracks.length,'Lista filtrata');let completed=0,unmatched=0;for(let index=0;index<tracks.length;index++){const track=tracks[index];setGlobalActionIcon(identifySpotifyButton,krIcon.spotify,'Ricerca Spotify ID',`${index+1}/${tracks.length}`);krProgress.update('Ricerca Spotify ID',index+1,tracks.length,spotifyFailureCountDetail(unmatched));try{let result;try{result=await post('spotify-identify',{id:track.id})}catch(error){if(!error.message.includes('Limite Spotify'))throw error;setGlobalActionIcon(identifySpotifyButton,krIcon.spotify,'Pausa Spotify','30 secondi');krProgress.update('Pausa limite Spotify',index+1,tracks.length,'Attendo 30 secondi');await wait(30000);result=await post('spotify-identify',{id:track.id})}Object.assign(track,result.track);completed++}catch(error){unmatched++;krProgress.update('Ricerca Spotify ID',index+1,tracks.length,spotifyFailureCountDetail(unmatched))}if(index<tracks.length-1)await wait(950)}identifySpotifyButton.disabled=false;bulkSpotifyButton.disabled=false;renderTracks();decorateSpotifyTracks();updateBulkSpotifyButton();krProgress.done('Ricerca Spotify ID completata',`${completed} ok${unmatched?` - ${unmatched} non riconosciuti`:''}`);toast(`${completed} Spotify ID salvati${unmatched?` - ${unmatched} non riconosciuti`:''}`)});

forceSpotifyButton.addEventListener('click',async()=>{const tracks=state.tracks.filter(track=>Number(track.id)>0);if(!tracks.length)return;if(!window.confirm(`Rifare Spotify ID per ${tracks.length} brani visibili?`))return;forceSpotifyButton.disabled=true;identifySpotifyButton.disabled=true;bulkSpotifyButton.disabled=true;krProgress.start('Forza ricerca Spotify ID',tracks.length,'Lista filtrata');let completed=0,errors=0;for(let index=0;index<tracks.length;index++){setGlobalActionIcon(forceSpotifyButton,krIcon.force,'Forza ricerca Spotify ID',`${index+1}/${tracks.length}`);krProgress.update('Forza ricerca Spotify ID',index+1,tracks.length,spotifyFailureCountDetail(errors));try{let result;try{result=await post('spotify-identify',{id:tracks[index].id,force:true})}catch(error){if(!error.message.includes('Limite Spotify'))throw error;setGlobalActionIcon(forceSpotifyButton,krIcon.force,'Pausa Spotify','30 secondi');krProgress.update('Pausa limite Spotify',index+1,tracks.length,'Attendo 30 secondi');await wait(30000);result=await post('spotify-identify',{id:tracks[index].id,force:true})}Object.assign(tracks[index],result.track);completed++}catch(error){errors++;krProgress.update('Forza ricerca Spotify ID',index+1,tracks.length,spotifyFailureCountDetail(errors))}if(index<tracks.length-1)await wait(950)}forceSpotifyButton.disabled=false;identifySpotifyButton.disabled=false;bulkSpotifyButton.disabled=false;setGlobalActionIcon(forceSpotifyButton,krIcon.force,'Forza ricerca Spotify ID sui brani visibili');renderTracks();decorateSpotifyTracks();updateBulkSpotifyButton();krProgress.done('Forza Spotify ID completata',`${completed} ok${errors?` - ${errors} errori`:''}`);toast(`${completed} brani forzati${errors?` - ${errors} errori`:''}`)});
...
let pendingMoveFolderTrackId=0;
async function openMoveFolderDialog(track){
  pendingMoveFolderTrackId=Number(track.id);
  const dialog=$('#move-folder-dialog'),select=$('#move-folder-select');
  $('#move-folder-track').textContent=`${track.artist||''} - ${track.title||''}`;
  select.innerHTML='<option value="">Caricamento cartelle...</option>';
  dialog.showModal();
  const data=await api('definitive-library-folders');
  const musicRoot=(state.bootstrap?.settings?.definitive_music_root||'E:\\LIBRERIA_MUSICALE').toUpperCase();
  const items=(data.items||[]).filter(item=>String(item.path||'').toUpperCase().startsWith(musicRoot));
  select.innerHTML='<option value="">Scegli cartella...</option>'+items.filter(item=>Number(item.depth)>0).map(item=>`<option value="${escapeHtml(item.path)}">${escapeHtml(item.tree_label||item.label)}</option>`).join('');
}
document.addEventListener('click',async event=>{const button=event.target.closest('[data-action="library-move-choose"]');if(!button)return;event.preventDefault();event.stopPropagation();const row=button.closest('.track-row');const track=state.tracks.find(item=>Number(item.id)===Number(row?.dataset.id));if(!track)return;try{await openMoveFolderDialog(track)}catch(error){toast(error.message)}});
$('#confirm-move-folder')?.addEventListener('click',async event=>{const button=event.currentTarget,folder=$('#move-folder-select')?.value||'';if(!pendingMoveFolderTrackId||!folder){toast('Scegli una cartella destinazione');return}button.disabled=true;try{const result=await post('track-move-folder',{id:pendingMoveFolderTrackId,folder});const index=state.tracks.findIndex(item=>Number(item.id)===Number(pendingMoveFolderTrackId));if(index>=0)state.tracks[index]=result.track;$('#move-folder-dialog').close();renderTracks();decorateSpotifyTracks();toast('File spostato nella cartella scelta')}catch(error){toast(error.message)}finally{button.disabled=false}});
